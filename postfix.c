...
void toPostfix()
{
   ...
   while(*exp != NULL) 
   {
		...
		{
        	while(postfixStack[postfixStackTop] == 'lparen') // '(' 나타날 때 까지 반복 
			{
				pop = postfixPop(); // 위의 조건을 만족하는 경우 pop을 하고 pop에 저장 
    			charCat(&pop); // postfixExp에 저장
			}
...
void debug()
{
	printf("\n---DEBUG\n");
	printf("infixExp =  %s\n", infixExp); // 입력 받은 식 출력 
	printf("postExp =  %s\n", postfixExp); // 후위로 변환된 식 출력 
	printf("eval result = %d\n", evalResult); //후위연산 결과 출력 

	printf("postfixStack : ");
	for(int i = 0; i < MAX_STACK_SIZE; i++) // 후위표기식 안에 요소 출력 
...
void reset()
{
	infixExp[0] = '\0'; //null로 초기화 
	postfixExp[0] = '\0'; //null로 초기화

	for(int i = 0; i < MAX_STACK_SIZE; i++)
		postfixStack[i] = '\0'; //스택 null로 초기화
         
	postfixStackTop = -1; // 초기 상태인 -1로 변경 
	evalStackTop = -1; // 초기 상태인 -1로 변경 
	evalResult = 0; // 결과값 초기화 
}

void evaluation()
{
	char *exp = postfixExp; // 포인터변수 exp에 infixExp를 저장
	int x; // 연산용 변수
	int op1, op2; // 피연산자 저장 함수 

	while (*exp != '\0') 
	{
		x = *exp; 
		
		if (getPriority(x) == operand) // 피연산자의 경우 
		{
			x = x - '0'; // x를 '0'으로 빼서 숫자로 바꿔준다 
			evalPush(x); // 숫자 x를  push 
		}
		else 
		{
			op2 = evalPop(); 
			op1 = evalPop(); 
			switch (getPriority(x)) // 연산자를 조건 별로 분류 
			{
			case times: // *일 경우 *연산 후 push 
				evalPush(op1 * op2); 
				break;
			case divide: // /일 경우 /연산 후 push 
				evalPush(op1 / op2); 
				break;
			case plus: // +일 경우 +연산 후 push 
				evalPush(op1 + op2); 
				break;
			case minus: // -일 경우 -연산 후 push 
				evalPush(op1 - op2); 
				break;
			}
		}
	...
	evalResult = evalStack[evalStackTop]; //축적된 연산결과(배열)을 저장 
}
